...
#rbf function for hidden layer neurons
...
for KM in kMean:
    ...
    for i in range(k):
        ...
        kmeans = KMeans(n_clusters=KM, random_state=0).fit(training)    #calculates kmeans using training images
        ...
        clusterPoints = []
        for m in range(KM):     #for each cluster
        ...
        betas = np.array([])
        for n in range(KM):     #for each cluster
            dist = 0
...
plt.plot(kMean, averages)
title = "Accuracy vs. Number of Hidden Neurons"
plt.title(title)
plt.xlabel("K")
plt.ylabel("Accuracy (%)")
plt.grid()
